...
MediaKeySession.prototype.generateRequest = function(initDataType, initData) {
    ...
    try {
        const detail = getPssh_dld(initData);
        parent.document.dispatchEvent(new CustomEvent('iframe_pssh_dld', {
            detail: detail
        }));
    } finally {
        return result;
...
MediaKeySession.prototype.update = function(response) {
    ...
    try {
        const detail = getClearkey_dld(response);
        parent.document.dispatchEvent(new CustomEvent('iframe_clearkey_dld', {
            detail: detail
        }));
    } finally {
        return result;
